...
| File | Action |
| --- | --- |
| `packages/{author}/customs/<tool>/` | **untouched** — the previous variant's package stays in the repo and keeps serving any production traffic it already has (only its TOURNAMENT slot is handed to the new version) |
