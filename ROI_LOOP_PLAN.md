...
| 5 | **Prevent fallback from overriding abstention** *(code finding verified; live configuration still to confirm)*. The current fallback path treats every zero bet amount alike, so a deliberate "no edge — do not bet" result can trigger another strategy. The shipped fixed-bet strategy checks the bankroll floor but not market price, edge, confidence, fees, or expected value. Both service configs default fallback to OFF, while the generic agent and skill configs default it to ON. Confirm the effective setting in every live deployment, default automatic fallback to OFF at every configuration layer, and make strategy results distinguish `BET`, `ABSTAIN`, and `ERROR`. `ABSTAIN` must be terminal; fallback may occur only after a genuine execution failure and only to a strategy that independently applies the required price, edge, fee, liquidity, and risk checks. Log the selected strategy and fallback reason, and cover each outcome with tests | Strategy | High if enabled in production; otherwise preventative | Med |
| 6 | **Judge tools on money in every benchmark** — beats the price? right when it disagrees? simulated profit? — and open issues on those signals too | Measurement | Compounding | Med |
| 7 | **Stricter promotion**: beats the price and repeats the win on a second run, measured on unfiltered price data | Measurement | Med | Small |
| 8 | **Stamp every bet with the tool version that produced it** — today no profit or loss is attributable to a tool version | Measurement | Med | Med |

**Start with 1 → 2 this week.** 3 and 4 are independent and can run in parallel. 5–8 follow. (Open PRs #433/#445 — computed daily-report tables + promote/demote — are early pieces of 6 and 7.)
