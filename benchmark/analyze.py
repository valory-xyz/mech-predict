...
def _active_tools_for_platform(
    ...
    Computed as the union of (benchmarked tools ∩ that deployment's
    ``VALID_TOOLS`` allow-list) across every deployment of ``platform``
...
def section_tool_deployment_status(
    ...
    the benchmarked tools intersected with that deployment's
    ``VALID_TOOLS`` allow-list. Failed-fetch deployments are called out
    so ``⚠️ unavailable`` is never confused with "no tools allowed".

    :param scores: parsed ``scores.json`` dict.
    :param valid: pre-fetched ``{deployment: [tool_names] | None}`` map.
...
def generate_report(  # pylint: disable=too-many-statements
    ...
    *,
    ...
    :param valid_tools: pre-fetched ``{deployment: [tool_names] | None}``
        ``VALID_TOOLS`` allow-list map used by the Tool Deployment Status
        section.
