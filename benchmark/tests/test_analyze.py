...
class TestSectionToolDeploymentStatusInverted:
    ...
    def test_active_tools_are_allow_listed(self) -> None:
        """Active tools are the benchmarked set intersected with VALID_TOOLS."""
...
class TestActiveToolsForPlatform:
    ...
    def test_full_failure_returns_none(self) -> None:
        """The platform's only deployment None -> caller shows all with a notice."""
        valid: dict[str, list[str] | None] = {"omenstrat Pearl": None}
        assert _active_tools_for_platform(valid, "omen", self._SCORES) is None

    def test_allow_list_drives_active_set(self) -> None:
        """Active set is the benchmarked tools that appear in VALID_TOOLS."""
        valid: dict[str, list[str] | None] = {"omenstrat Pearl": ["tool-a", "tool-c"]}
    ...
    def test_polystrat_failure_returns_none(self) -> None:
        ...
        valid: dict[str, list[str] | None] = {"polystrat Pearl": None}
        assert _active_tools_for_platform(valid, "polymarket", self._SCORES) is None

    def test_non_benchmarked_allow_entries_are_ignored(self) -> None:
        """Allow-list names a tool with no scores -> it can't become active."""
        valid: dict[str, list[str] | None] = {
            "omenstrat Pearl": ["tool-a", "not-benchmarked"]
        }
        active = _active_tools_for_platform(valid, "omen", self._SCORES)
    ...
    def test_benchmarked_universe_unions_rolling_and_alltime(self) -> None:
        ...
        filtered out — provided it is allow-listed.
        """
        ...
        valid: dict[str, list[str] | None] = {
            "omenstrat Pearl": ["tool-a", "tool-b", "tool-new"]
        }
    ...
    def test_rolling_scores_optional_preserves_alltime_only_behaviour(self) -> None:
        ...
        """
        all_time: dict[str, Any] = {"by_tool": {"tool-a": {}, "tool-b": {}}}
        valid: dict[str, list[str] | None] = {"omenstrat Pearl": ["tool-a", "tool-b"]}
