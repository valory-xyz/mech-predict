...
#
# ------------------------------------------------------------------------------
"""Tests for benchmark/stats_loop.py.
...
from benchmark.stats_loop import (BRIER_REGRESSION_THRESHOLD,
                                  RELIABILITY_FLOOR, VALID_N_PER_WINDOW_FLOOR,
                                  _load_json, _window_iso, build_issue_body,
                                  build_issue_title, triage, write_state)
...
class TestRegressionSizeGate:
    """Δ Brier > 0.040 is required (calibrated 2026-05-29)."""
...
class TestDuplicateIssueSuppression:
    """Sustained regressions file at most one issue.

    Once an issue opens, subsequent days stay silent until the
    regression resolves (gates 2-4 stop firing).
    """
...
def test_gate_precedence(case: Dict[str, Any]) -> None:
    """Higher-priority gates short-circuit lower ones."""
    cur = _stats(
