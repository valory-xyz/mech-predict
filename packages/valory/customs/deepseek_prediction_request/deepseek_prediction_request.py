...
"""The implementation of the deepseek_prediction_request package."""
...
class LLMClient:
    ...
    def completions(
        ...
        if self.llm_provider == "openrouter":
            ...
            print(f"**************** Provider response: {response_provider}")
            response = LLMResponse()
            response_message: ChatCompletionMessage = response_provider.choices[
                0
            ].message
            response.content = response_message.content
            print(f"response.content = {response.content}")
            response_usage: CompletionUsage = response_provider.usage
            response.usage.prompt_tokens = response_usage.prompt_tokens
            response.usage.completion_tokens = response_usage.completion_tokens
            return response
...
ALLOWED_TOOLS = ["deepseek-prediction-online", "deepseek-prediction-offline"]
...
def extract_json_string(text):
    ...
    if isinstance(text, dict):
        print(f"the input is a dictionary")
        return json.dumps(text, indent=2)
    pattern = r"(\{[^}]*\})"
    matches = re.findall(pattern, text)
    transformed_text = matches[0].replace("json", "")
    print(f"transformed_text={transformed_text}")
...
def run(**kwargs) -> Tuple[str, Optional[str], Optional[Dict[str, Any]], Any]:
    """The callable for the deepseek_prediction_request tool."""
    print(f"Running deepseek_prediction_request with {kwargs=}.")
    """Run the task"""
    tool = kwargs["tool"]
    ...
    with LLMClientManager(kwargs["api_keys"], engine):
        ...
        if counter_callback:
            print(f"counter_callback parameter={counter_callback}")
        ...
        additional_information = ""
        if tool == "deepseek-prediction-online":
