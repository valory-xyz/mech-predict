...
What v2 adds over factual_research-v1
-------------------------------------
For Polymarket markets, the mech request carries a ``request_context`` dict
(built by the trader) whose ``description`` field holds the market's
**resolution rules** — the authoritative criteria for how the market resolves
YES/NO. The trader fetches that text from the Gamma API and forwards it on the
request (see trader PR #989); v2 simply READS it and injects it into the
**Reframe** step (so sub-questions target the exact resolution criteria) and the
**Estimate** step (so the probability maps to the real resolution condition
rather than a loose reading of the question).

The tool itself never contacts Polymarket — fetching is the trader's job — so no
price/odds/volume signal can leak in: the trader forwards only the resolution
text, preserving the "facts, not odds" philosophy. v2 degrades to exact v1
behaviour whenever no resolution rules are present (Omen markets, which omit the
field, or any request without a ``description``).
...
ALLOWED_TOOLS = ["factual_research-v2"]
