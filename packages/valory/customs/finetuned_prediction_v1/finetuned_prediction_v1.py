...
#
# ------------------------------------------------------------------------------
"""Mech tool v1: fine-tuned 14B Qwen prediction model via vLLM, calibrated slot.

Successor of ``predict-fine-tuned-calibrated`` (finetuned_prediction).

What changed vs the parent
--------------------------
The parent tool registered the ``predict-fine-tuned-calibrated`` mode against
the ft-serve virtual served name ``qwen-14b-fine-tuned-calibrated`` -- a proxy
that applies a Platt calibrator on top of the raw fine-tuned model's output.
Tournament data (issue #411) showed the Platt calibrator to be miscalibrated:
all 607 tournament rows produce p_yes in [0.17, 0.36] (stdev = 0.025), compared
to p_yes in [0.009, 1.000] (stdev = 0.245) for the sibling ``predict-fine-tuned``
tool which uses the same model without the calibrator.  The Brier for the
calibrated slot (0.2640 all-time, 0.3421 T-1) is substantially worse than for
the uncalibrated sibling (0.1939 all-time, 0.2503 T-1) despite identical
evidence sources and prompts, confirming a server-side Platt miscalibration.

Fix: this version requests the raw fine-tuned served name (``qwen-14b-fine-tuned``)
instead of the calibrated one, bypassing the miscalibrated Platt proxy.  The
single-axis change is the ``SERVED_MODEL_FINE_TUNED_CALIBRATED`` constant at the
top of this file; all other logic is identical to the parent.

Three modes (base / fine-tuned / fine-tuned-calibrated-v1)
-----------------------------------------------------------
This package is registered under the single tool name
``predict-fine-tuned-calibrated-v1``.  Unlike the parent (which hosted all three
modes in one package), this package serves only the calibrated slot, keeping the
diff minimal and the ablation attributable.  The base and raw fine-tuned modes
continue to be served by the parent package.

Parsing parity
--------------
``extract_json`` / ``parse_p_yes`` are vendored from
``fine_tuning/src/fine_tuning/training/reward.py`` (the single source of truth
there).  Behaviourally identical to the parent; only the module-level docstring
and the SERVED_MODEL_FINE_TUNED_CALIBRATED constant differ.  See the pinned
commit on the parsing block below.
"""
...
# This package serves a single mode: the calibrated slot (v1).  The tool NAME
# is the only selector -- ``predict-fine-tuned-calibrated-v1`` routes here.
TOOL_FINE_TUNED_CALIBRATED = "predict-fine-tuned-calibrated-v1"
ALLOWED_TOOLS = [TOOL_FINE_TUNED_CALIBRATED]

# vLLM --served-model-name for this mode.
# Parent used "qwen-14b-fine-tuned-calibrated" (ft-serve's Platt-calibrated proxy).
# v1 uses "qwen-14b-fine-tuned" (the raw fine-tuned model) to bypass the
# miscalibrated Platt calibrator -- the single mechanism change for issue #411.
SERVED_MODEL_BASE = "qwen-14b-base"
SERVED_MODEL_FINE_TUNED = "qwen-14b-fine-tuned"
# Single axis change: request raw fine-tuned model instead of Platt proxy.
SERVED_MODEL_FINE_TUNED_CALIBRATED = SERVED_MODEL_FINE_TUNED

MODEL_BY_TOOL = {
    TOOL_FINE_TUNED_CALIBRATED: SERVED_MODEL_FINE_TUNED_CALIBRATED,
}

# Self-hosted vLLM OpenAI-compatible endpoint (same endpoint as the parent).
...
def run(**kwargs: Any) -> Union[MaxCostResponse, MechResponse]:
    """Run the fine-tuned prediction tool v1 (predict-fine-tuned-calibrated-v1).

    Expected kwargs (supplied by the mech task-execution layer):
      - tool:        must be ``predict-fine-tuned-calibrated-v1``.
    ...
    The served model is ``qwen-14b-fine-tuned`` (raw fine-tuned, no Platt
    calibrator). The endpoint comes from api_keys[`finetuned_endpoint`],
    falling back to the VLLM_ENDPOINT constant.
