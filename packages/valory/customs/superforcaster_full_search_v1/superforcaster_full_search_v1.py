...
What superforcaster_full_search-v1 does (vs superforcaster_full_search)
------------------------------------------------------------------------
v1 inherits full-page scraping (top organic results via readability +
markdownify) and adds the evidence-reliability screen introduced by
superforcaster-polymarket-v4 as a MANDATORY reasoning field. The screen
forces the model to classify retrieved sources TYPE A (dated within the
resolution window, or directly confirming the criterion) vs TYPE B
(undated, standing pages, or perennial content) and to check whether any
TYPE A evidence DIRECTLY confirms the exact resolution condition -- not
just that the topic is active. This disrupts the dominant failure mode
observed in issue #440: the model treating topical relevance as
criterion-satisfaction confirmation, producing overconfident YES on
narrow-criterion questions ("Will X say Y during event Z?", "Will word W
be in headlines this week?") that resolve NO.

The response is an OpenAI structured-output object (PredictionResult),
not prose + a trailing JSON blob. Only the four numeric fields are
returned on-chain per the mech protocol, satisfying the trader contract.

One coherent mechanism: evidence-reliability screen before probability
formation, implemented via four sub-steps mirroring v4 (4a-4d) but
applied over the richer scraped-page evidence that full_search provides.
"""
...
class PredictionResult(BaseModel):
    """superforcaster_full_search-v1 structured output.

    Reasoning fields are declared before the four numeric fields so that
    structured outputs condition the numbers on the completed chain-of-thought.
    Do NOT reorder: Pydantic will not complain but the calibration would
    degrade (numbers emitted before reasoning is finished).

    The evidence_reliability_screen is the key addition vs the parent tool:
    it forces criterion-specificity checking before probability formation,
    disrupting the mechanism where topical relevance is treated as
    criterion-satisfaction confirmation (issue #440 primary hypothesis).
    """
    ...
    # Criterion-specificity screen: gate-visible fix for issue #440.
    # Forces the model to classify sources TYPE A/B and check that at least
    # one TYPE A source directly confirms the resolution criterion, rather
    # than anchoring on topical relevance alone.
    ...
    # chain-of-thought. Do not reorder or alphabetize.
    p_yes: float = Field(
        ...,
        ge=0.0,
        le=1.0,
...
def _null_prediction_response(exc: Exception, api_keys: Any) -> MechResponseWithKeys:
    ...
    failure -- rate-limit exhaustion, a permanent API error, a schema
        ...
        def execute() -> Union[MaxCostResponse, MechResponseWithKeys]:
            ...
            try:
                ...
                if retries_left["openai"] <= 0 and retries_left["openrouter"] <= 0:
                    print(f"[superforcaster_full_search-v1] rate-limit exhausted: {e}")
                ...
                print(f"[superforcaster_full_search-v1] permanent failure: {e}")
                return _null_prediction_response(e, api_keys)
...
DEFAULT_OPENAI_SETTINGS = {
    # 4096 tokens: the six reasoning fields (including the evidence-reliability
    # screen) need headroom to complete before the four numeric fields are
    # emitted; otherwise a truncated completion raises LengthFinishReasonError.
    "max_tokens": 4096,
    "limit_max_tokens": 4096,
    "temperature": 0,
}
DEFAULT_OPENAI_MODEL = "gpt-4.1-2025-04-14"
ALLOWED_TOOLS = ["superforcaster_full_search-v1"]
MAX_SOURCES = 5
...
# Cap on the rendered evidence block to bound prompt size.
...
- facts: compress the sources (including any scraped page content) and useful background
  into specific, relevant core factual points. Do NOT draw conclusions about how a fact
  influences the answer here.
...
def _parse_completion(  # pylint: disable=too-many-arguments,too-many-positional-arguments
    ...
    Uses client.beta.chat.completions.parse to guarantee the completion is
    well-formed JSON matching the schema. RateLimitError is deliberately not
    caught here so the with_key_rotation decorator can rotate keys on a
    rate-limit hit.
    ...
    :param response_format: Pydantic model class for structured-output schema.
    :param temperature: sampling temperature (0 = deterministic).
    :param max_tokens: maximum tokens to generate.
    :param retries: number of retry attempts on transient failure.
    ...
    :raises RuntimeError: if all retries are exhausted without a parse.
    """
    attempt = 0
    ...
    while attempt < retries:
        try:
            ...
            print(f"[superforcaster_full_search-v1] Attempt {attempt + 1} failed: {e}")
            last_error = e
            time.sleep(delay)
            attempt += 1
...
def _fetch_page_content(
    ...
    try:
        ...
        print(f"[superforcaster_full_search-v1] Failed to fetch {url}: {e}")
        return None, None
    ...
    with ThreadPoolExecutor(max_workers=_SCRAPE_POOL_WORKERS) as pool:
        ...
        for fut in as_completed(future_to_item):
            ...
            try:
                ...
                print(
                    f"[superforcaster_full_search-v1] Scrape error "
                    f"for {item['link']}: {e}"
                )
                continue
            if text:
...
def _cap_evidence_block(
    ...
    Serper orders organic results by relevance so trailing drops are cheapest.
    If the block still exceeds the budget once all organic items are gone, the
    ...
    :return: rendered evidence string, with a truncation marker if items were
        dropped.
    """
    ...
    **kwargs: Any,
    ...
    with OpenAIClientManager(openai_api_key) as llm_client:
        ...
        if source_content is not None:
            ...
            print(
                f"Scraped {len(captured_pages)}/{min(MAX_SOURCES, len(organic_data))} "
                f"pages."
            )
        ...
        print("Getting prompt response (structured outputs)...")
        # OpenAI structured outputs: the model reasons INTO the PredictionResult
        # schema fields (including the mandatory evidence_reliability_screen) and
        # the SDK returns a validated object. Only the four numeric fields are
        # serialised to the on-chain result, so no reasoning prose can leak
        # into the trader's flat-json.loads parse path.
        ...
        print(
            f"[superforcaster_full_search-v1] Result: p_yes={prediction.p_yes}, "
