...
"""Unit tests for superforcaster_full_search_v1's structured-output contract.

Hard Constraint 0 (from CLAUDE.md) mandates that any new-version tool ships
with at least three test assertions mirroring the v4 reference set:
  - test_on_chain_result_is_flat_json_loads_parseable
  - test_uses_structured_parse_not_raw_create
...
from packages.valory.customs.superforcaster_full_search_v1.superforcaster_full_search_v1 import (
    PredictionResult,
    _parse_completion,
    run,
)
...
def _make_mock_api_keys() -> MagicMock:
    ...
    mock = MagicMock()
    mock.__getitem__ = lambda self, key: key_map[key]
    mock.get = lambda key, default="": key_map.get(key, default)
    return mock
...
class TestStructuredOutputContract:
    """v1 uses OpenAI Structured Outputs; the on-chain result is always clean JSON."""

    @patch(f"{V1_MODULE}.OpenAIClientManager")
    @patch(f"{V1_MODULE}.fetch_additional_sources")
    def test_uses_structured_parse_not_raw_create(
        ...
        run(
            tool="superforcaster_full_search-v1",
...
class TestPredictionResultSchema:
    ...
    def test_numeric_fields_are_declared_last(self) -> None:
        ...
        numbers after the reasoning chain is what preserves v1's calibration.
    ...
    def test_evidence_reliability_screen_is_present(self) -> None:
        """evidence_reliability_screen field must exist (criterion-specificity fix #440)."""
        assert "evidence_reliability_screen" in PredictionResult.model_fields

    def test_evidence_reliability_screen_is_before_numerics(self) -> None:
        """evidence_reliability_screen must precede the numeric fields.

        The screen forces criterion-specificity checking before probability
        formation; placing it after the numerics would break the reasoning order.
        """
        fields = list(PredictionResult.model_fields)
        screen_idx = fields.index("evidence_reliability_screen")
        p_yes_idx = fields.index("p_yes")
        assert screen_idx < p_yes_idx, (
            "evidence_reliability_screen must be declared before p_yes "
...
class TestFailurePathContract:
    ...
    @patch(f"{V1_MODULE}._parse_completion", side_effect=RuntimeError("boom"))
    @patch(f"{V1_MODULE}.OpenAIClientManager")
    @patch(f"{V1_MODULE}.fetch_additional_sources")
...
class TestParseCompletion:
    ...
    def test_refusal_retries_then_raises_runtimeerror(self) -> None:
        """A refusal (parsed=None) retries and raises RuntimeError."""
        client = MagicMock()
