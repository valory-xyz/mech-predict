...
"""Unit tests for superforcaster-full-search-v2's structured-output contract."""
...
class TestStructuredOutputContract:
    """v2 uses OpenAI Structured Outputs; the on-chain result is always clean JSON."""
    ...
    def test_on_chain_result_is_flat_json_loads_parseable(
        ...
        assert "facts" not in parsed and "word_mention_check" not in parsed
...
class TestPredictionResultSchema:
    ...
    def test_word_mention_check_is_before_numeric_fields(self) -> None:
        ...
        Structured outputs emit fields in declaration order; the screen must be
        filled before the model commits to p_yes.
        """
...
class TestFailurePathContract:
    ...
    @patch(f"{V2_MODULE}._parse_completion", side_effect=RuntimeError("boom"))
    @patch(f"{V2_MODULE}.fetch_additional_sources")
    @patch(f"{V2_MODULE}.OpenAIClientManager")
