...
ALLOWED_TOOLS = ["superforcaster-market-aware-v1"]
MAX_SOURCES = 5
...
class PredictionResult(BaseModel):
    ...
    # Issue #493: the four sub-checks of evidence_reliability_screen all push the
    # estimate DOWN, so an unsupported NO is never challenged the way (d) challenges
    # an unsupported YES. These two fields are that missing NO-side check.
    disconfirmation_check: str = Field(
        ...,
        description=(
            "MANDATORY NO-side counterpart to the criterion-specificity check "
            "in `evidence_reliability_screen`, completed AFTER that screen and "
            "BEFORE any probability. That check stops an unsupported YES; this "
            "one stops an unsupported NO. Absence of confirming evidence is "
            "NOT disconfirming evidence: a question that resolves in the "
            "future cannot yet have a source confirming its criterion, so "
            "'no source confirms it' is the normal state of every open market "
            "and is never on its own a reason for a tail probability. Answer "
            "in this order. (1) DISCONFIRMING SOURCE: name the specific TYPE A "
            "evidence that makes the criterion FAIL, or write 'none'. (2) GAP "
            "TO THRESHOLD: when the criterion is a threshold on an observable "
            "quantity, state the latest observed value WITH its as-of date, "
            "the threshold, the gap as a percentage of that value, and how far "
            "the quantity has typically moved over a window of the same "
            "length as the one that remains. A threshold inside that typical "
            "move is an ordinary event, not a tail one. State whether the "
            "criterion resolves on a "
            "single end-of-window reading or on ANY touch during the window; a "
            "touch is strictly more likely than a close and must be priced "
            "higher. (3) REPEAT-EVENT BASE RATE: when the criterion is an act "
            "the subject performs routinely (a stock phrase, a recurring "
            "topic), state how often it has occurred in comparable past "
            "instances rather than whether a source announces it in advance."
        ),
    )
    base_rate_floor: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description=(
            "The lower bound your `disconfirmation_check` justifies, committed "
            "as a number BEFORE you forecast. When (1) named no disconfirming "
            "TYPE A source AND (2)/(3) did not put the criterion well outside "
            "ordinary movement or ordinary frequency, this is the category "
            "base rate you stated in `evidence_reliability_screen` -- not a "
            "tail value. Go below it only when you named a disconfirming "
            "source or a quantified gap that supports doing so, and say which "
            "in `disconfirmation_check`. This is a reasoning commitment, not a "
            "clamp: nothing downstream overrides your probability with it."
        ),
    )
...
7. `disconfirmation_check` and `base_rate_floor` - MANDATORY, and completed BEFORE you form
any probability. Step 6 stops an unsupported YES; this step stops an unsupported NO. A question
that resolves in the future cannot yet have a source confirming its criterion, so "nothing
confirms it" is the normal state of every open market and is not on its own a reason for a tail
probability. Follow that field's instructions in order: name the disconfirming TYPE A source or
write "none"; for a threshold on an observable quantity, state the latest observed value and its
as-of date, the gap to the threshold as a percentage, the quantity's typical move over a window
of the same length as the one that remains, and whether the market resolves on a close or on ANY
touch; for an act the subject performs routinely, state its frequency in comparable past
instances. Then commit `base_rate_floor` as a number.

8. `independent_reasoning` and `p_independent` - Reason to a probability from YOUR evidence
alone and commit to it here, BEFORE considering any supplied market price.

9. `market_reconciliation` - If a market price was supplied, say where it sits relative to
`p_independent`. Moving away from it requires naming a specific TYPE A source.

10. `aggregation` - Aggregate your considerations. Do not summarize or repeat previous points;
...
11. `reflection` - Reflect on your tentative answer, performing sanity checks and mentioning any
...
12. `p_yes`, `p_no`, `confidence`, `info_utility` - Output your final prediction. `p_yes` is the
...
def _parse_completion(
    ...
    while attempt < retries:
        try:
            ...
            print(f"[superforcaster-market-aware-v1] Attempt {attempt + 1} failed: {e}")
            time.sleep(delay)
            attempt += 1
            last_error = e
...
def _fetch_page_content(
    ...
    try:
        ...
        print(f"[superforcaster-market-aware-v1] Failed to fetch {url}: {e}")
        return None, None
    ...
    with ThreadPoolExecutor(max_workers=_SCRAPE_POOL_WORKERS) as pool:
        ...
        for fut in as_completed(future_to_item):
            ...
            try:
                ...
                print(
                    f"[superforcaster-market-aware-v1] Scrape error for {item['link']}: {e}"
                )
                continue
            if text:
                item["content"] = text
    ...
    for item in organic_data:
        ...
        if mode == "raw":
            ...
            try:
                ...
                print(
                    "[superforcaster-market-aware-v1] Failed to clean cached "
...
def _extract_market_context(request_context: Any) -> Dict[str, Any]:
    ...
    if rejected:
        print(
            "[superforcaster-market-aware-v1] Market context fields supplied "
...
def _flagged_null_result(
    *,
    ...
    """
    print(
        f"[superforcaster-market-aware-v1] {context}: empty retrieval"
...
def run(**kwargs: Any) -> Union[MaxCostResponse, MechResponse]:
    ...
    with OpenAIClientManager(openai_api_key) as llm_client:
        ...
        if scan_truncated:
            print(
                f"[superforcaster-market-aware-v1] Scan window exhausted: "
                f"prompt is {len(prompt)} chars, scanned the first "
                f"{_MAX_SCAN_CHARS}; tier={tier}, query: {search_query!r}"
            )
        elif tier == "raw":
            print(
                "[superforcaster-market-aware-v1] No question clause found; "
                f"using capped prompt head as the search query: {search_query!r}"
            )
        elif tier == "clause":
            print(
                f"[superforcaster-market-aware-v1] Free-text prompt (tier={tier}); "
        ...
        if market_context:
            print(
                f"[superforcaster-market-aware-v1] Market context supplied: "
                f"{sorted(market_context)}"
            )
        else:
            print("[superforcaster-market-aware-v1] No market context; blind mode.")
        print(f"\n{prediction_prompt=}\n")
        messages = [
        ...
        print(f"[superforcaster-market-aware-v1] === FACTS ===\n{prediction.facts}")
        print(
            f"[superforcaster-market-aware-v1] === REASONS_NO ===\n{prediction.reasons_no}"
        )
        print(
            f"[superforcaster-market-aware-v1] === REASONS_YES ===\n"
            f"{prediction.reasons_yes}"
        )
        print(
            f"[superforcaster-market-aware-v1] === AGGREGATION ===\n"
            f"{prediction.aggregation}"
        )
        print(
            f"[superforcaster-market-aware-v1] === REFLECTION ===\n"
            f"{prediction.reflection}"
        )
        print(
            f"[superforcaster-market-aware-v1] Result: p_yes={prediction.p_yes}, "
