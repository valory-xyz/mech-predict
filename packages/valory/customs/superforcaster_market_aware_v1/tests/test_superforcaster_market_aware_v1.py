...
"""Unit tests for superforcaster_market_aware_v1: page scrape, capture/replay, fallbacks."""
...
import packages.valory.customs.superforcaster_market_aware_v1.superforcaster_market_aware_v1 as module
from packages.valory.customs.superforcaster_market_aware_v1.superforcaster_market_aware_v1 import (
    OpenAIClientManager,
    PredictionResult,
...
class TestOpenAIClientManager:
    ...
    def test_context_manager_returns_client_instance(self) -> None:
        ...
        with patch(
            "packages.valory.customs.superforcaster_market_aware_v1.superforcaster_market_aware_v1.OpenAI"
        ) as MockClient:
...
SF_MODULE = "packages.valory.customs.superforcaster_market_aware_v1.superforcaster_market_aware_v1"
...
def _make_prediction_stub() -> PredictionResult:
    ...
    return PredictionResult(
        ...
        disconfirmation_check="no disconfirming source; gap 1% of spot",
        base_rate_floor=0.35,
    ...
    def test_validator_rejects_probabilities_that_do_not_sum_to_one(self) -> None:
        ...
        with pytest.raises(ValidationError):
            PredictionResult(
                ...
                disconfirmation_check="none; gap 1%",
                base_rate_floor=0.3,
...
class TestBlindVersusMarketAwareRun:
    ...
    def test_blind_mode_prompt_has_no_market_block(
        ...
        result = run(
            tool="superforcaster-market-aware-v1",
            model="gpt-4o",
...
class TestDisconfirmationCheck:
    """The NO-side counterpart to the criterion-specificity check (issue #493)."""

    def test_check_precedes_every_probability(self) -> None:
        """Both new fields are declared before the first number the model emits."""
        order = list(PredictionResult.model_fields)
        assert order.index("evidence_reliability_screen") < order.index(
            "disconfirmation_check"
        )
        assert order.index("disconfirmation_check") < order.index("base_rate_floor")
        assert order.index("base_rate_floor") < order.index("p_independent")

    def test_check_states_that_absence_of_evidence_is_not_disconfirmation(
        self,
    ) -> None:
        """The mechanism the fix targets is named explicitly in the field."""
        desc = PredictionResult.model_fields["disconfirmation_check"].description or ""
        assert "NOT disconfirming evidence" in desc
        assert "cannot yet have a source confirming its criterion" in desc

    def test_check_asks_for_the_three_sub_answers(self) -> None:
        """All three sub-answers survive any later edit of the description."""
        desc = PredictionResult.model_fields["disconfirmation_check"].description or ""
        for clause in (
            "DISCONFIRMING SOURCE",
            "GAP TO THRESHOLD",
            "REPEAT-EVENT BASE RATE",
        ):
            assert clause in desc

    def test_touch_is_priced_above_a_close(self) -> None:
        """Barrier questions must not be read as end-of-window closes."""
        desc = PredictionResult.model_fields["disconfirmation_check"].description or ""
        assert "ANY touch during the window" in desc
        assert "strictly more likely than a close" in desc

    def test_floor_is_a_bounded_number_and_not_a_clamp(self) -> None:
        """The floor is a committed number, and the field says it is not enforced."""
        field = PredictionResult.model_fields["base_rate_floor"]
        assert field.annotation is float
        assert any(
            getattr(m, "ge", None) == 0.0 for m in field.metadata
        ), "base_rate_floor must declare ge=0"
        assert any(
            getattr(m, "le", None) == 1.0 for m in field.metadata
        ), "base_rate_floor must declare le=1"
        assert "not a" in (field.description or "")
        assert "clamp" in (field.description or "")

    def test_prompt_carries_the_matching_numbered_step(self) -> None:
        """A schema field with no prompt step is silently skipped by the model."""
        template = module.PREDICTION_PROMPT
        assert "7. `disconfirmation_check` and `base_rate_floor`" in template
        assert "8. `independent_reasoning` and `p_independent`" in template

    def test_new_fields_never_reach_the_delivered_payload(self) -> None:
        """The on-chain key set is unchanged from the parent (issue #493)."""
        desc_keys = ("disconfirmation_check", "base_rate_floor")
        source = Path(module.__file__).read_text(encoding="utf-8")
        delivered = source.split("result = json.dumps(")[1].split("used_params")[0]
        for key in desc_keys:
            assert f'"{key}"' not in delivered


class TestSuperforcasterSourceContent:
    """Verify superforcaster_market_aware_v1 captures and replays source_content correctly."""
...
# FINAL implementation as merged in PR #459) ---
...
class TestParsePrompt:
    """Test parse_prompt -> ParsedPrompt(question, query, tier) (issue #455)."""
    ...
    def test_boilerplate_prefix_is_dropped_from_query(self) -> None:
        """The query anchors at the question clause, dropping instruction text."""
    ...
    def test_double_quotes_are_stripped_from_query_only(self) -> None:
        """Quoted spans become exact-match Serper terms; drop them."""
        prompt = (
...
class TestClauseSelection:
    ...
    def test_market_question_wins(self, prompt: str, expected_start: str) -> None:
        """Leading/trailing instruction and clarifying questions must not win."""
    ...
    def test_no_market_verb_clause_falls_back_to_best_score(self) -> None:
        """With no market-verb candidate, the best-scoring clause wins."""
...
class TestEmptyRetrievalGuard:
    ...
    def test_empty_live_retrieval_returns_flagged_null(
        ...
        """Organic + PAA both empty -> null payload with markers, no LLM call."""
        mock_fetch.return_value = MagicMock(json=lambda: EMPTY_SERPER_RESPONSE)
        result = run(
            tool="superforcaster-market-aware-v1",
            model="gpt-4o",
        ...
        assert parsed["researchability"] is None  # market-aware extras stay null
        used_params = result[4]
        assert used_params["empty_retrieval"] is True
        assert used_params["parse_tier"] == "clause"
        assert used_params["source_content"]["serper_response"] == (
            EMPTY_SERPER_RESPONSE
        )

    @patch(f"{SF_MODULE}.OpenAIClientManager")
    @patch(f"{SF_MODULE}.fetch_additional_sources")
    def test_cached_replay_empty_retrieval_returns_flagged_null(
        self, mock_fetch: MagicMock, mock_client_mgr: MagicMock
    ) -> None:
        """The cached-replay branch fires the same guard without a live fetch."""
        result = run(
            tool="superforcaster-market-aware-v1",
            model="gpt-4o",
            prompt=FREE_TEXT_PROMPT,
    ...
    def test_reshaped_serper_body_is_an_error_not_a_flagged_null(
        ...
        """A 200 body without the organic key surfaces as an error null."""
        mock_fetch.return_value = MagicMock(json=lambda: {"message": "quota"})
        result = run(
            tool="superforcaster-market-aware-v1",
            model="gpt-4o",
            prompt=FREE_TEXT_PROMPT,
    ...
    def test_cached_replay_missing_organic_key_is_an_error(
        ...
        """A corrupted cache entry raises, mirroring the live shape check."""
        result = run(
            tool="superforcaster-market-aware-v1",
            model="gpt-4o",
            prompt=FREE_TEXT_PROMPT,
    ...
    def test_empty_retrieval_skips_scraping(
        ...
        """The guard fires BEFORE the page-scrape step (no wasted fetches)."""
        mock_fetch.return_value = MagicMock(json=lambda: EMPTY_SERPER_RESPONSE)
        run(
            tool="superforcaster-market-aware-v1",
            model="gpt-4o",
    ...
    def test_trader_request_sends_extracted_question_to_serper(
        ...
        """End-to-end pin: a trader request searches the bare question."""
        ...
        # the LLM prompt (result[1]) carries the bare question, not the template
