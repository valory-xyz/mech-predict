...
What v4 changes vs superforcaster-polymarket-v1
------------------------------------------------
v4 addresses overconfident YES predictions (brier 0.81 in the >=0.9 p_yes bucket) caused by
the prediction-LLM-call stage anchoring on (a) prediction-market odds embedded in search
results and (b) forward-looking intent language without applying a materialization discount.

Two structural changes:
1. PREDICTION_PROMPT step 4 gains a mandatory evidence-reliability screen that requires the
   model to identify and discard prediction-market-odds (circular self-referential) and to
   apply a 40-60% materialization discount to forward-looking intent language before forming
   its tentative probability.
2. The OUTPUT_FORMAT instruction is changed from "only a single JSON object" to "after
   completing steps 1-6, output the JSON" so the reasoning chain executes and the discount
   protocol fires.  max_tokens is raised from 500 to 1500 to accommodate the full chain-of-
   thought.

Gate-visible: both changes are downstream of source_content injection and exercised by PR-CI's
cached W-2 replay.
"""
...
DEFAULT_OPENAI_SETTINGS = {
    # Raised from 500 to 1500 to allow the full chain-of-thought reasoning
    # (steps 1-6) to execute before the final JSON is emitted (hypothesis: prior
    # truncation at 500 tokens caused the model to skip the reasoning steps and
    # anchor directly on salient evidence numbers without applying a discount).
    "max_tokens": 1500,
...
mandatory and must be completed before you form your tentative probability:
  ...
  After completing (a) and (b), aggregate your remaining considerations. Investigate how
  the competing factors and mechanisms interact and weigh against each other. We have
  detected that you overestimate world conflict, drama, violence, and crises due to news'
  negativity bias, which doesn't necessarily represent overall trends or base rates.
  Similarly, we also have detected you overestimate dramatic, shocking, or emotionally
  charged news due to news' sensationalism bias. Therefore adjust for news' negativity
  bias and sensationalism bias by considering reasons to why your provided sources might be
  biased or exaggerated. Think like a superforecaster. Use <thinking></thinking> tags for
  this section of your response.
