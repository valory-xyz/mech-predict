...
------------------------------------------------------------------------
v2 added a resolution-criterion specificity check in step 6 of the prompt, but
OUTPUT_FORMAT told the model to emit ONLY JSON, so the chain-of-thought
(including step 6) was skipped and p_yes was formed from topical relevance
alone. max_tokens=500 would also truncate any chain-of-thought attempt.
Result: systematic p_yes >= 0.93 on "Will X say Y?" and "Will X be in
headlines?" markets that resolved NO (confirmed in Issue #412 C-1 worst-miss
rows: 10/10 deliveries had good evidence but bad reasoning).

v5 fix (single mechanism -- Issue #412): force the existing 7-step chain-of-
thought to execute visibly before p_yes is formed, by (a) changing OUTPUT_FORMAT
to require the full analysis chain followed by the JSON at the end (not "only
JSON"), and (b) raising max_tokens from 500 to 2000 so the chain-of-thought
fits. _extract_prediction_json() then pulls the trailing JSON from the response
so the trader receives a flat json.loads()-parseable string.
"""
...
DEFAULT_OPENAI_SETTINGS = {
    "max_tokens": 2000,  # raised from 500: chain-of-thought + JSON fits in ~600-900 tokens
...
OUTPUT_FORMAT
* Complete steps 1-7 above in full (emitting all tagged sections). Then, immediately after your
  <answer></answer> tag, output a single JSON object on its own line. No markdown fences, no
  extra text after the JSON. The JSON is parsed by Python's "json.loads()" so it must be valid.
...
* The p_yes in the JSON MUST match the value in your <answer></answer> tag.
* Example final lines: <answer>*0.35*</answer>
{{"p_yes": 0.35, "p_no": 0.65, "confidence": 0.7, "info_utility": 0.6}}
"""


# Returned when the LLM response cannot be parsed (trader requires flat json.loads()).
...
def _extract_prediction_json(text: str) -> str:
    """Extract the trailing JSON prediction object from a chain-of-thought completion.

    v5 requires the model to emit its analysis chain (steps 1-7) BEFORE the
    JSON (Issue #412 fix). This function locates the trailing JSON object so
    the trader receives a flat json.loads()-parseable string, as required by
    decision_maker_abci/behaviours/decision_receive.py::_get_decision.

    :param text: raw LLM completion (chain-of-thought + trailing JSON).
    :return: the JSON string if found, otherwise NULL_PREDICTION.
    """
    if not text:
        return NULL_PREDICTION
    text = text.strip()
    # Fast path: entire response is already valid JSON (model skipped CoT).
    try:
        json.loads(text)
        return text
    except (json.JSONDecodeError, ValueError):
        pass
    # Locate the last '{' and walk forward to its matching '}' to extract JSON.
...
def run(**kwargs: Any) -> Union[MaxCostResponse, MechResponse]:
    ...
    with OpenAIClientManager(openai_api_key) as llm_client:
        ...
        # v5 fix (Issue #412): extract trailing JSON from chain-of-thought response
        # so the trader's json.loads() receives a flat, parseable string.
        prediction_json = _extract_prediction_json(extracted_block)
        used_params = {
            "model": model,
            "temperature": temperature,
            "max_tokens": max_tokens,
        ...
        return prediction_json, prediction_prompt, None, counter_callback, used_params
