...
What superforcaster-polymarket-v5 does (vs superforcaster-polymarket-v4)
------------------------------------------------------------------------
v5 adds programmatic utterance-question detection at the prompt-construction
stage (gate-visible) and injects an UTTERANCE_QUESTION_ADDENDUM when the
question matches the pattern "Will X say/tweet/post/declare/announce Y at
event Z?".

The v4 evidence_reliability_screen TYPE A/B classification failed for this
question class: historical utterance records ("Trump has said X 487 times")
were treated as TYPE A evidence (directly confirming the criterion), preventing
the 20-40% base-rate fallback from activating. The result was systematic
overconfident YES (p_yes=0.93-0.99) on questions that resolved NO at a 90%
error rate (Issue #422, current CID, n=40 utterance rows, Brier=0.3543 vs
non-utterance n=4, Brier=0.0635).

The addendum adds three sub-steps to evidence_reliability_screen:
(e) Historical utterance records are TYPE B -- they document past behavior at
    past events and do NOT confirm the criterion for a specific named future
    event.
(f) TYPE A for an utterance question requires a direct, dated report confirming
    that X said the specific phrase at the specific named future event.
(g) Base rate when TYPE A evidence is absent: 20-50% YES. The question resolves
    on a single future event, not on aggregate lifetime frequency.

One mechanism, one site (prompt_construction), one hypothesis (Issue #422 H1).
"""
...
class PredictionResult(BaseModel):
    ...
    evidence_reliability_screen: str = Field(
        ...
        description=(
            ...
            "For utterance questions, also apply sub-steps (e), (f), (g) if "
...
# Matches utterance questions: "Will X say/tweet/post/declare/announce Y ...?"
# These require special TYPE A/B classification because historical utterance
# records document past behavior and are TYPE B for a future-event criterion.
UTTERANCE_QUESTION_RE = re.compile(
    r"\bwill\b.{0,120}\b(say|tweet|post|state|declare|announce|mention|endorse|claim|express|affirm)\b",
    re.IGNORECASE | re.DOTALL,
)

# Injected at prompt-construction stage when UTTERANCE_QUESTION_RE matches.
# Targets Issue #422 H1: historical utterance records misclassified as TYPE A,
# preventing the base-rate fallback for "Will X say Y at event Z?" questions.
UTTERANCE_QUESTION_ADDENDUM = """
UTTERANCE QUESTION: This question asks whether a specific statement will be
made at a specific future event. Apply the following additional classification
rules inside your evidence_reliability_screen BEFORE forming any probability:

(e) Historical utterance records are TYPE B. Sources reporting "X has said Y
    N times before", "X frequently uses phrase Z", or tallying past speeches
    document past behavior at past events. They do NOT confirm that X will say
    the specific phrase at the named future event. Do NOT classify them as
    TYPE A on the basis of historical frequency alone.

(f) TYPE A for an utterance question requires a direct, dated report confirming
    that X said the specific phrase at the specific named future event. If all
    sources contain only historical utterance frequency data, you have ZERO
    TYPE A evidence for the resolution criterion.

(g) Base rate when TYPE A evidence is absent: 20-50% YES. The question resolves
    on a single future event, not on aggregate lifetime frequency. Apply this
    anchor before any upward adjustment from historical frequency data.
"""
...
def run(**kwargs: Any) -> Union[MaxCostResponse, MechResponse]:
    ...
    with OpenAIClientManager(openai_api_key) as llm_client:
        ...
        # Utterance-question detection (Issue #422 H1 fix: TYPE A/B misclassification).
        # "Will X say/tweet/post/announce Y at event Z?" questions require explicit
        # guidance that historical utterance records are TYPE B -- past behavior at
        # past events, not confirmation of the criterion at the named future event.
        # Without this addendum the evidence_reliability_screen classifies frequency
        # data as TYPE A, producing overconfident YES (Issue #422: 9/10 high-p_yes
        # utterance predictions resolved NO; current-CID Brier=0.3543 vs 0.0635).
        is_utterance_question = bool(UTTERANCE_QUESTION_RE.search(question))
        ...
        if is_utterance_question:
            # Append utterance-specific TYPE A/B guidance (e)(f)(g) so the
            # evidence_reliability_screen correctly classifies historical records
            # as TYPE B and applies the 20-50% base-rate fallback.
            prediction_prompt += UTTERANCE_QUESTION_ADDENDUM
        print(f"\n{prediction_prompt=}\n")
        messages = [
