...
-----------------------------------------------------------------------
v5 fixes the free-text input contract to honor the advertised tool metadata,
which describes the input as a plain-text question (issue #455). v4 silently
broke for any caller that is not the valory trader: the trader-template regex
in extract_question() returned the full prompt on a mismatch, and Serper
deterministically returns organic: [] for queries longer than ~150 chars,
causing the LLM to forecast from priors alone.

Two gate-visible changes (both downstream of source_content injection):

(5a) Query extraction fix (derive_search_query): when the trader-template
     regex does not match, compress the prompt into a short Serper query --
     prefer the first question-sentence (ending with '?'); otherwise use the
     first _MAX_SEARCH_QUERY_LEN characters. The LLM question field still
     receives the full prompt for complete context.

(5b) Empty-retrieval guard: after the Serper call (live or cached replay),
     if both organic and peopleAlsoAsk are empty, return a flagged null
     prediction (p_yes=0.5, p_no=0.5, confidence=0.0, info_utility=0.0) so
     the requester can detect and discount it. Prior behavior was to silently
     pass an empty <background> block to the LLM and emit a prior-only p_yes.

v4's evidence-reliability screen (4a-4d), structured outputs, token budget,
...
# Compiled regex matching the valory trader's internal prompt template.
# When this matches, the captured group is a concise question string that also
# serves as the Serper search query. When it does not match (free-text input),
# extract_question() returns the full prompt for LLM context and
# derive_search_query() compresses it to a short query for Serper.
_TRADER_TEMPLATE_RE = re.compile(r'question\s+"(.+?)"\s+and\s+the\s+`yes`', re.DOTALL)

# Serper degrades on queries longer than ~150 chars (empirically verified in
# issue #455: a 580-char query produced organic: []). Free-text queries are
# capped at this length before submission.
...
def extract_question(prompt: str) -> str:
    """Extract the question text for the LLM from the prompt.

    Tries the trader-template regex (_TRADER_TEMPLATE_RE) first. On a match,
    ...
    # produced organic: []. The LLM question still uses the full prompt here so
    # it has all resolution criteria, source, and deadline context.
    match = _TRADER_TEMPLATE_RE.search(prompt)
    if match:
...
def derive_search_query(prompt: str, extracted_question: str) -> str:
    ...
    :return: a search query string no longer than _MAX_SEARCH_QUERY_LEN chars.
    """
...
def run(**kwargs: Any) -> Union[MaxCostResponse, MechResponse]:
    ...
    with OpenAIClientManager(openai_api_key) as llm_client:
        ...
        if source_content is not None:
            ...
            # Empty-retrieval guard (5b): signal explicitly rather than letting
            # the LLM forecast from a blank <background> block.
            if not organic_data and not misc_data:
                print(
                    "[superforcaster-polymarket-v5] cached replay: empty retrieval"
