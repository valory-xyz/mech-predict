...
class TestUtteranceQuestionDetection:
    """v5 detects utterance questions and injects the TYPE A/B addendum."""

    def test_utterance_re_matches_say(self) -> None:
        """UTTERANCE_QUESTION_RE matches 'Will X say Y' patterns."""
        assert UTTERANCE_QUESTION_RE.search("Will Trump say Fake News during cabinet?")

    def test_utterance_re_matches_tweet(self) -> None:
        """UTTERANCE_QUESTION_RE matches 'Will X tweet Y' patterns."""
        assert UTTERANCE_QUESTION_RE.search("Will Elon tweet about tariffs this week?")

    def test_utterance_re_matches_announce(self) -> None:
        """UTTERANCE_QUESTION_RE matches 'Will X announce Y' patterns."""
        assert UTTERANCE_QUESTION_RE.search("Will the Fed announce a rate cut?")

    def test_utterance_re_no_match_on_non_utterance(self) -> None:
        """UTTERANCE_QUESTION_RE does not match generic prediction questions."""
        assert not UTTERANCE_QUESTION_RE.search("Will inflation exceed 3% in 2025?")
        assert not UTTERANCE_QUESTION_RE.search("Will the S&P 500 close above 6000?")

    def test_utterance_addendum_contains_type_b_rule(self) -> None:
        """UTTERANCE_QUESTION_ADDENDUM contains the TYPE B classification rule."""
        assert "TYPE B" in UTTERANCE_QUESTION_ADDENDUM
        assert "20-50%" in UTTERANCE_QUESTION_ADDENDUM

    @patch(f"{V5_MODULE}.OpenAIClientManager")
    @patch(f"{V5_MODULE}.fetch_additional_sources")
    def test_utterance_addendum_injected_for_utterance_question(
        self, mock_fetch: MagicMock, mock_client_mgr: MagicMock
    ) -> None:
        """run() appends UTTERANCE_QUESTION_ADDENDUM for utterance questions."""
        mock_fetch.return_value = MagicMock(json=lambda: FAKE_SERPER_RESPONSE)
        mock_client = MagicMock()
        mock_client.client.beta.chat.completions.parse.return_value = (
            _mock_parse_response()
        ...
        assert "UTTERANCE QUESTION" in user_content
        assert "TYPE B" in user_content

    @patch(f"{V5_MODULE}.OpenAIClientManager")
    @patch(f"{V5_MODULE}.fetch_additional_sources")
    def test_no_utterance_addendum_for_non_utterance_question(
        self, mock_fetch: MagicMock, mock_client_mgr: MagicMock
    ) -> None:
        """run() does NOT append the addendum for non-utterance questions."""
        mock_fetch.return_value = MagicMock(json=lambda: FAKE_SERPER_RESPONSE)
        mock_client = MagicMock()
        mock_client.client.beta.chat.completions.parse.return_value = (
            _mock_parse_response()
        ...
        messages = parse.call_args.kwargs["messages"]
        user_content = messages[1]["content"]
        assert "UTTERANCE QUESTION" not in user_content
...
class TestPredictionResultSchema:
    ...
    def test_numeric_fields_are_declared_last(self) -> None:
        ...
        numbers after the reasoning chain is what preserves v5's calibration.
