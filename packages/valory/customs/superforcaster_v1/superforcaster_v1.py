...
class PredictionResult(BaseModel):
    ...
    reflection: str = Field(
        ...,
        description=(
            "Sanity checks and finalisation. Apply the four checks: "
            "EVIDENCE BAR, CONFIDENCE COUPLING, NUMERIC QUESTIONS, "
            "WEEKLY OCCURRENCE QUESTIONS. Check for "
...
# Fix #315: weekly-occurrence base-rate anchoring added as check 4 in reflection
ALLOWED_TOOLS = ["superforcaster-v1"]
...
BEFORE FINAL ANSWER -- apply all four checks:
...
4. WEEKLY OCCURRENCE QUESTIONS: For questions of the form "Will [subject] say/
   post/tweet/write/mention '[term]' this week?" or "Will '[term]' be in the
   headlines this week?", first establish a BASE RATE before consulting the
   evidence:
   - "Will [public figure] say '[word]'?" -- if [word] is a common English word
     or a frequently-discussed policy topic for that figure, base rate is
     0.40-0.65 (public figures speak extensively each week). If [word] is a
     rare proper noun or a highly specific phrase, base rate is 0.10-0.25.
   - "Will '[term]' be in the headlines?" -- the resolution criterion is
     typically narrow (term must appear as a top-line keyword in a specific
     source, not just in article bodies). Base rate: 0.15-0.40 regardless of
     how prominent the topic is in general coverage.
   Anchor your p_yes to this base rate first, then adjust with evidence.
   Do NOT treat general media coverage of a topic as confirming the specific
   headline criterion is met. Do NOT treat the absence of a specific dated
   announcement as proof an event will not occur if the base rate is high.
...
def _parse_completion(
    ...
    while attempt < retries:
        try:
            ...
            print(f"[superforcaster_v1] Attempt {attempt + 1} failed: {e}")
            time.sleep(delay)
            attempt += 1
...
def run(**kwargs: Any) -> Union[MaxCostResponse, MechResponse]:
    ...
    with OpenAIClientManager(openai_api_key) as llm_client:
        ...
        print(f"[superforcaster_v1] === FACTS ===\n{prediction.facts}")
        print(f"[superforcaster_v1] === REASONS_NO ===\n{prediction.reasons_no}")
        print(f"[superforcaster_v1] === REASONS_YES ===\n{prediction.reasons_yes}")
        print(f"[superforcaster_v1] === AGGREGATION ===\n{prediction.aggregation}")
        print(f"[superforcaster_v1] === REFLECTION ===\n{prediction.reflection}")
        print(
            f"[superforcaster_v1] Result: p_yes={prediction.p_yes}, "
